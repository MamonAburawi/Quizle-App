            ...
            quizResultRepository.searchQuizResults(query)
                .onSuccess(
                    onDataSuccess = { quizResults ->
                        _state.update { it.copy(quizResults = quizResults, isLoading = false, error = null, initMessage = "") }
                    }
                )
                .onFailure {
                    _state.update { it.copy(isLoading = false, error = it.error, quizResults = emptyList(), initMessage = "") }
                }
        }
