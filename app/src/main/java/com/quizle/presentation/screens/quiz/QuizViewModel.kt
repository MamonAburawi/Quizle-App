...
//    private val topicCode = 66 // no topic
        ...
        when(action){
            is QuizAction.NextQuestionButtonClick -> {
                val newIndex = (_state.value.currentQuestionIndex + 1)
                    .coerceAtMost(maximumValue = _state.value.quizQuestions.lastIndex)
            ...
            is QuizAction.ExitQuizDialogConfirm -> {
                viewModelScope.launch {
                    async { userRepository.recordUserEvent(RecordUserEvent.QUIZ_END)}.await() // log activity
    ...
    private suspend fun saveUserAnswers() {
            ...
            quizQuestionRepository.saveUserAnswers(answers)
                .onSuccess {}
                .onFailure {}
    }
    ...
    private suspend fun saveResult(){
            val answers = _state.value.answers
            val questions = _state.value.quizQuestions
    ...
    fun setUpQuiz(){
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true, error = null, quizQuestions = emptyList(), loadingMessage = "Setting up quiz...") }
            async { applyQuizSettings() }.await()
            async { getQuizTopic(topicId) }.await()
            async { getQuizQuestions(topicId) }.await()
            async { userRepository.recordUserEvent(RecordUserEvent.QUIZ_START)}.await() // log activity
    ...
    private fun selectOption(selectedIndex: Int) {
        ...
        val quizQuestions = _state.value.quizQuestions
